...
router.post('/sucursales', async (req, res) => {
  ...
  const { nombre, direccion, googleMapsUrl } = req.body;

  try {
    console.log('🔍 [DEBUG-ROUTES-STAFF] POST /sucursales - llamando a crearSucursal (servicio, no es supabase.from directo) con:', { nombre, direccion, googleMapsUrl });
    const sucursal = await crearSucursal({ nombre, direccion, googleMapsUrl });
...
router.put('/sucursales/:id', async (req, res) => {
  ...
  try {
    console.log('🔍 [DEBUG-ROUTES-STAFF] PUT /sucursales/:id - llamando a actualizarSucursal (servicio, no es supabase.from directo) con id:', req.params.id, '| datos:', { nombre, direccion, googleMapsUrl });
    const sucursal = await actualizarSucursal(req.params.id, { nombre, direccion, googleMapsUrl });
