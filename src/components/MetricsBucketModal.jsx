...
export default function MetricsBucketModal({ title, filtros, onClose }) {
  ...
  useEffect(() => {
    ...
    adminFetch(`/api/metrics/detalle${params.toString() ? `?${params}` : ''}`)
      ...
      .catch(err => {
        console.error('❌ [DEBUG-COMPONENT-MetricsBucketModal] Error cargando los chats de esta categoría:', err);
        setError(err.message || 'Error cargando los chats de esta categoría.');
      })
