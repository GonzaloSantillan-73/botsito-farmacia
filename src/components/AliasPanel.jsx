...
export default function AliasPanel() {
  ...
  const handleSave = async () => {
    ...
    await supabase
      .from('app_settings')
      .upsert({ key: 'alias', value: alias, updated_at: new Date().toISOString() }, { onConflict: 'key' });
    setSaving(false);
  };
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">
        Configurá el Alias institucional de la farmacia. Podés usar la etiqueta <strong>{"{{ALIAS}}"}</strong> en la plantilla de respuesta rápida si la personalizaste.
      </p>
